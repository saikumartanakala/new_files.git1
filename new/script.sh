...
install_node() {
    ...
    while true; do
        read -p "Enter the Node.js version compatible with glibc 2.26 (e.g., 16.20.0): " NODE_VERSION
...
# Function to install modern PHP versions compatible with glibc 2.26
install_php() {
    while true; do
        read -p "Enter the PHP version you want to install (e.g., 7.4 or 8.0): " PHP_VERSION
        if [[ "$PHP_VERSION" =~ ^[0-9]+\.[0-9]+$ ]]; then
            break
        else
            echo "Invalid version format! Example: 7.4"
        fi
    done
